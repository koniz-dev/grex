...
- name: Generate changelog (Linux)
  run: ./scripts/linux/maintenance/generate_changelog.sh ${{ github.event.release.tag_name }}

- name: Generate changelog (Windows)
  run: .\scripts\windows\maintenance\generate_changelog.ps1 ${{ github.event.release.tag_name }}
...
```bash
# Post-commit hook example (Linux)
#!/bin/bash
...
```powershell
# Post-commit hook example (Windows)
if (git diff HEAD~1 --name-only | Select-String "pubspec.yaml") {
